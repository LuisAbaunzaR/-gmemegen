...
def get_args():
    ...
    if args is None:
        ...
        parser.add_argument(
            '-d', '--db-host',
            default=os.environ.get('host', '35.225.43.109'),
            help='Host address of the database.')
        parser.add_argument(
            '-n', '--db-name',
            default=os.environ.get('sunn', 'memegen'),
            help='Name of the database.')
        parser.add_argument(
            '-u', '--db-user',
            default=os.environ.get('postgr', 'postgres'),
            help='User used to connect to the database.')
        parser.add_argument(
            '-p', '--db-pass',
            default=os.environ.get('passwo', 'password'),
